...
void WriteCmd()
{
	uint8_t i = 0;
	for(i=0; i<27; i++){
		HAL_I2C_Mem_Write(&hi2c ,0x78,0x00,I2C_MEMADD_SIZE_8BIT,CMD_Data+i,1,0x100);
	}
}
//向设备写控制命令
...
void displayF16x32_dig(uint8_t x,uint8_t y,uint8_t v){
    OLED_Set_Pos(x,y);
    for (int j = y; j < y+4; j++)
    {
        OLED_Set_Pos(x,j);
        for (int i = 0; i < 16; i++)
        {
            OLED_WR_DATA(F16x32_dig[v][i+16*(j-y)]);
        }
    }
}
